...
for (tile in mymap){
	...
	if (mymap[tile] == 7){ // 7 used for walls
		...
		myTerrainTiles.push({color: 0xffff00, x:X, y: -1, z:Z, texture: 'textures/wire.jpg', wire: true});
		if (mapLineCounter==9&&mapCurrentLine==6){
...
myTerrainTiles.forEach( function(item, index, array){
	...
	if (item.hasOwnProperty('texture')){
		...
		mytexture.wrapS = THREE.RepeatWrapping; // You do not need to set `.wrapT` in this case
		mytexture.offset.x = ( 2 * Math.PI );
		var mymaterial = new THREE.MeshBasicMaterial({ map: mytexture });
	} else {
	        var mymaterial = new THREE.MeshBasicMaterial({color: item.color, transparent: true, opacity:0});
	}
        tile = new THREE.Mesh( mygeometry, mymaterial );
        tile.position.set(item.x, item.y, item.z);
        scene.add(tile);
	myDisplayedTiles.push(tile);
        if (item.hasOwnProperty('wire')){
...
var imagePrefix = "textures/city/city_";
var directions  = ["front", "back", "top", "floor", "left", "right"];
//var directions  = ["back", "left", "top", "floor", "right", "front"];
var imageSuffix = ".jpg";
...
var mymaterial = new THREE.MeshBasicMaterial({map: mytexture, transparent: true, opacity: 0.5});
...
var mygeometry = new THREE.CubeGeometry( 10, 200, 10 );
var mygeometry = new THREE.CubeGeometry( 10, 200, 10 );
mytexture = THREE.ImageUtils.loadTexture('textures/tower.jpg');
mytexture.wrapS = mytexture.wrapT = THREE.ClampToEdgeWrapping;
mytexture.repeat.set( 1, 1 );
var mymaterial = new THREE.MeshBasicMaterial({map: mytexture});
TwinTowerNorth = new THREE.Mesh( mygeometry, mymaterial );
TwinTowerNorth.position.set(15,-100,6-4);
scene.add(TwinTowerNorth);
TwinTowerSouth = new THREE.Mesh( mygeometry, mymaterial );
TwinTowerSouth.position.set(-10,-100,6+4);
scene.add(TwinTowerSouth);
